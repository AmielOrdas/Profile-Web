...
export default function ProjectCard(props: ProjectCardProps) {
  const {
    ...
    colorTheme = "purple",
    marginLeftPicture = "5",
    icons = new Array(20).fill(null).map(function (_, index) {
      return {
        icon: (
          <div
  ...
  return (
    ...
    >
      {/* Icons Row hover:border border-red-600  */}

      {/* Icons Row */}
      <div className="w-full flex flex-wrap gap-3 mb-4 justify-">
          ...
          return (
            <div key={index} className="gradient-border-hover p-1 rounded-full">
              <a
                ...
                className={`w-10 h-10 bg-${colorTheme}-300 rounded-full flex items-center justify-center transition-all duration-200`}
              >
                {iconObj.icon}
              </a>
            </div>
      ...
      <div className="flex flex-row">
        ...
        <div className="w-[40%] flex items-center justify-center relative">
          <div
            className={`w-50 h-50 bg-${colorTheme}-200 rounded-full absolute opacity-50 translate-x-6 translate-y-2`}
          ></div>
          <div
            className={`w-50 h-50 bg-${colorTheme}-300 rounded-full absolute opacity-75 translate-x-2 translate-y-2`}
          ></div>
          <div
            className={`w-50 h-50 bg-${colorTheme}-500 rounded-full flex items-center justify-center text-3xl relative   ml-${marginLeftPicture} `}
          >
            {emoji}
          </div>
        </div>
